...
# Making the Figure 1 row 1 and row 2 align: font, color and shape:
# for the manuscript
plottstore <-
  function(x, type=c("coef","fitted"), bubble=FALSE, col=NULL, xlab="State",
    ...
    if (type == "coef") {
      ...
      if (bubble) {
        ...
        #if(x$pen=="none") qq <- qq+theme(legend.position = "none")
        print(qq)
      } else {
...
# Figure for multiple models: on penalized models
# A function to produce the 1x5 grid of store run outcome

# organize the data into ggplot format:
taugamaGendat <- function(objList){
  # objList is the store run for all the models for a response variable y
  ...
  #sv$predictor <- rep(str_to_title(paste0(y,".")),nrow(sv))
  ...
  for (l in 1:lv) {
    ...
    # divide by var(latent predictor): stdev of latent variable)
    smatt <- matt/sqrt(var(predict(objList[[l]]))+1)
    obsize <- store:::coefObservations(formula,data,id)
    #taugamavals <- as.vector(t(matt)) # original 
    taugamavals <- as.vector(t(smatt)) # standardized
  ...
  #bigDF$response <- gl(1,nrow(bigDF),labels=str_to_title(y))
...
taugamaGrid<- function(objList,vbls){
  ...
  #vals <- DF[,"taugamavals"]
  #mv <- mean(DF[,"mdpointval"],na.rm=T)
  # col<-designer.colors(col=c("red","white","blue"),
  #                       x=c(0,-min(vals,na.rm=T)/(max(vals,na.rm=T)-min(vals,na.rm=T)),1))
  #DF$predictor <- factor(DF$predictor,levels=str_to_title(paste0(vbls,".")))
  DF$predictor <- factor(DF$predictor,levels=paste0("x = ", str_to_title(vbls)))
  #qq <- ggplot(DF,aes(x=state,y=trait,col=taugamavals,size=obsize))
  require(ggplot2)
  require(latex2exp)
  ...
  #qq <- qq + xlab(TeX("State")) + ylab(TeX("Trait")) 
