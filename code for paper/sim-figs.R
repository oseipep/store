...
# Figure 2 in manuscript: comparing the AICs
set1 = readRDS(paste0(simfolder, "Set1.rds"))
set2 = readRDS(paste0(simfolder, "Set2.rds"))

hh1 <- cbind(sapply(set1, function(v) AIC(v$simple)),
            sapply(set1, function(v) AIC(v$pen)),
            sapply(set1, function(v) AIC(v$unpen)))

hh2 <- cbind(sapply(set2, function(v) AIC(v$simple)),
             sapply(set2, function(v) AIC(v$pen)),
             sapply(set2, function(v) AIC(v$unpen)))
...
# Making Figures 3 and 4 of the manuscript: the boxplots
# latent response:
frs <- function(r,s,p) 
  0.17-0.15*sign(r-4)*abs(r-4)^p-0.72*sign(s-r)*abs(s-r)^p

# Compute MSE for setting 1:
ucoef1 <- t(sapply(set1, function(v) coef(v$unpen)))[,1:31]
pcoef1 <- t(sapply(set1, function(v) coef(v$pen)))[,1:31]
...
for (i in 1:length(nams)){
  ...
  if (substr(nams[i],1,3)=="dev"){
    ...
    tv <- frs(r=m1,s=m2,p)-frs(r=m1,s=m1,p)
    umse <- mean((ucoef1[,nams[i]]-tv)^2)
    pmse <- mean((pcoef1[,nams[i]]-tv)^2)
  }
  #-------------
...
ucoef2 <- t(sapply(set2, function(v) coef(v$unpen)))[,1:31]
pcoef2 <- t(sapply(set2, function(v) coef(v$pen)))[,1:31]
...
c1 <- as.numeric(substr(nams[2:7],4,4))
c2 <- as.numeric(substr(nams[8:31],4,4))
c3 <- as.numeric(substr(nams[8:31],6,6))
cc1 <- paste0("$\\tau_",c1,"$")
cc2 <- paste0("$\\gamma_{",c2,", ",c3,"}")
cc <- paste0("$\\alpha$")
nn <- c(cc,cc1,cc2)
#-----------------
...
p <- 1
for (i in 1:length(nams)){
  if(substr(nams[i],1,3)=="ice") tv <- frs(4,4,p)
  if (substr(nams[i],1,3)=="med"){
    m <- as.numeric(substr(nams[i],4,4))
    tv <- frs(r=m,s=m,p)-frs(4,4,p)
  }
...
df1 <- cbind(sapply(set1, function(v) v$unpen$fixed.edf),
           sapply(set1, function(v) v$pen$fixed.edf))

df2 <- cbind(sapply(set2, function(v) v$unpen$fixed.edf),
             sapply(set2, function(v) v$pen$fixed.edf))
